...
Public Sub AssociateText()
    'Associate Bobopad to plain text
    ...
    SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + ".TXT", "", "BoboPad plain text"
...
Public Sub AddSCviewer()
    'make Bobopad default Sourcecode viewer for IE
...
Public Sub AddShortCutSendTo()
    ...
    sShortcutPath = SpecialFolder(9) + "\BoboPad.lnk"
    If IsObject(sh) Then
        Set link = sh.CreateShortcut(sShortcutPath)
        If IsObject(link) Then
            link.Description = "A better note pad"
